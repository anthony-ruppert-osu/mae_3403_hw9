    ...
    def ImportFromFile(self, data):
        ...
        while i < len(data):
            ...
            if L.find('#') == 0:
                ...
                i = self.importUnits(data, i, self.truss)
            elif L.find('pipe') >= 0:
                # read in a pipe and increase i to end of pipe section
                i = self.importPipe(data, i, self.truss)
                pass
            elif L.find('loop') >= 0:
                i = self.importLoop(data, i, self.truss)
                pass
            elif L.find('node') >= 0:
                i = self.importNode(data, i, self.truss)
                pass
...
class TrussView():
    ...
    def drawLinks(self, truss=None):  # jes
        scene = self.scene
    ...
    def drawNodes(self, truss=None, scene=None):  # jes
        if scene is None:
            scene = self.scene
        ...
        for n in truss.nodes:
            ...
            # make a tool tip
            q_units = truss.units.FlowRate
            p_units = truss.units.Pressure
            tooltip = 'Node {}: {} \n'.format(n.getName(), n.fitting.type)
            p = n.P_Head if truss.SI else truss.units.CFPressure * n.P_Head
            if n.IsSprinkler:
                tooltip += 'K = {:0.2f}\n'.format(n.K)
            if n.ExtFlow is not None and n.ExtFlow != 0.0:
                Q = n.ExtFlow if truss.SI else n.ExtFlow * truss.units.CFFlowRate
                tooltip += 'Qin = {:0.2f} ({})\n'.format(Q, q_units)
            ...
            self.drawAnArrow(startX=a.x, startY=a.y, endX=b.x, endY=b.y, stArrow=(n.ExtFlow > 0.0),
                             endArrow=(n.ExtFlow < 0.0),
